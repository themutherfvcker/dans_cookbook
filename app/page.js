...
function Hero() {
  return (
    <section className="bg-gradient-to-br from-yellow-200 via-amber-100 to-white border-b">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20 grid lg:grid-cols-2 gap-10 items-center">
        <div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-gray-900">
            Edit or Generate images with simple text
          </h1>
          <p className="mt-4 text-gray-700 text-lg">
            Text→Image and Image→Image powered by Gemini 2.5 Flash Image Preview.
            Quick, consistent, credit-based.
          </p>
          <div className="mt-6 flex gap-3">
            <a
              href="#generator"
              className="inline-flex items-center px-5 py-2.5 rounded-md text-white bg-yellow-600 hover:bg-yellow-700 shadow-sm"
            >
              Try it now
            </a>
            <Link
              href="/pricing"
              className="inline-flex items-center px-5 py-2.5 rounded-md border border-gray-300 hover:bg-gray-50"
            >
              View Pricing
            </Link>
          </div>
        ...
        <div className="relative">
          ...
          />
          <div className="absolute -bottom-3 -right-3 bg-white border shadow px-3 py-1.5 rounded-md text-xs">
...
// --------------------------------------
// Generator (same logic as /generator)
// --------------------------------------
const STYLE_CHIPS = [
...
function Generator({ onBalance }) {
  ...
  // Drag & drop (only when image tab active)
  useEffect(() => {
  ...
  async function onGenerate() {
    ...
    try {
      ...
      onBalance?.(typeof j.balance === "number" ? j.balance : balance)
  ...
  return (
    <section id="generator" className="py-10 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4">
          <div className="bg-white shadow-sm rounded-lg p-4 sm:p-6 space-y-5">
            ...
            <p className="text-xs text-gray-500">Aspect & strength are hints; your API can read them from <code>meta</code>.</p>
          </div>
        </div>
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <Header balance={topBalance} />
      <Hero />
      <Generator onBalance={setTopBalance} />
      <footer className="border-t">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-sm text-gray-500 flex flex-wrap items-center justify-between gap-3">
          <p>© {new Date().getFullYear()} Nano Banana. All rights reserved.</p>
          <div className="flex gap-5">
            <Link href="/pricing" className="hover:text-gray-700">Pricing</Link>
            <a href="https://github.com/themutherfvcker/ai-image-app" target="_blank" rel="noreferrer" className="hover:text-gray-700">GitHub</a>
          </div>
        </div>
      </footer>
    </div>
